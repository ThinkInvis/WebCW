...
WebCW.Modules.Units.Turret = function(args) {
	...
	_this.OnPostUpdate = function(gt,dt,larg) {
		...
		if(tgts.length > 0 && _this.fireAccum <= 0) {
			...
			//_this.Mesh.children[0].children[0].children[0].children[0].rotation.x = Math.acos((tgtp[0]-_this.Position.x)/tgt[0]);
			//_this.Mesh.children[0].children[0].children[0].children[0].rotation.y = Math.PI;
			//_this.Mesh.children[0].children[0].children[0].children[0].lookAt(_this.Game.IndexToVec3(tgti));
	...
	_this.Game.UnsafeCode.FBXLoader.load( './WebCW/Models/turret.fbx', function ( object ) {
		...
		_this.Mesh.children[0].children[0].children[0].children[0].up.set(0, 0, 1);
